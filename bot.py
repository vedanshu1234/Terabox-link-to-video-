...
async def fetch_terabox_info(url: str) -> dict:
    # Cookie hai toh TeraboxDL use karo, nahi hai toh seedha fallback API
    if COOKIE:
        try:
            from TeraboxDL import TeraboxDL  # type: ignore
            ...
            pass  # TeraboxDL nahi hai, fallback chalega
        except Exception as e:
            logger.warning("TeraboxDL fail, fallback try karta hoon: %s", e)

    # Fallback: public API (bina cookie ke bhi kaam karta hai)
    return await _fallback_api(url)
