...
async def fetch_info(url):
    ...
    if RAPIDAPI_KEY:
        try:
            ...
            # Response format:
            # { status: "success", data: { structure: { file_name, size, download_url, direct_link, dlink } } }
            if isinstance(data, dict) and data.get("status") == "success":
                s = data.get("data", {}).get("structure", {})
                logger.info("Structure keys: %s", list(s.keys()))

                # Priority: dlink > download_url > direct_link
                # dlink = actual terabox CDN link (best for download)
                # direct_link = worker URL (may fail)
                # download_url = another worker URL
                dlink    = s.get("dlink") or ""
                dl_url   = s.get("download_url") or ""
                dir_link = s.get("direct_link") or ""

                # Prefer actual terabox/freeterabox CDN links over worker links
                def is_direct_cdn(u):
                    return any(x in u for x in ["d.terabox.app", "d.terabox.com",
                                                  "freeterabox.com", "terabox-url-fixer",
                                                  "d8.freeterabox", "d1.freeterabox"])

                dl = ""
                for candidate in [dlink, dl_url, dir_link]:
                    if candidate and candidate.startswith("http"):
                        dl = candidate
                        break

                if dl:
                    fname = s.get("file_name") or "video.mp4"
                    fsize = _parse_size(s.get("size") or s.get("sizebytes") or "0")
                    logger.info("API 1 (viewer) success ✅ dl=%s", dl[:80])
                    return {
                        "file_name":     fname,
                        "file_size":     fsize,
                        "download_link": dl,
                    ...
                    logger.warning("API 1: no usable download link in structure: %s", str(s)[:300])
        except Exception as e:
...
async def _send_file(update, ctx, info, caption):
    ...
    # Terabox CDN ke liye special headers
    dl_headers = {
        "User-Agent": (
